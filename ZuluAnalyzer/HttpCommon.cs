...
namespace ZuluAnalyzer
{
    ...
    public class HttpCommon
    {
        ...
        public void setProxy(string strProxyIP, int nProxyPort, string strProxyID, string strProxyPass)
        {
            WebProxy proxy = new WebProxy(strProxyIP, nProxyPort);
            if (strProxyID != "")
            {
                proxy.Credentials = new NetworkCredential(strProxyID, strProxyPass);
            }
        ...
        public bool sendRequest(bool bAsync = false, string parameter = "",  string contentType = "application/x-www-form-urlencoded", FileStream fileStream = null )
        {
            initialize();

            if (true == bAsync)
                return sendAsyncRequest(parameter, contentType);

            return sendSyncRequest(parameter, contentType, fileStream);
        }

        private bool sendSyncRequest(string parameter, string contentType, FileStream fileStream)
        {
            if (null == m_Request)
                return false;
        ...
        private bool sendAsyncRequest(string parameter, string contentType)
        {
            return true;
        }
